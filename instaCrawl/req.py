...
user_insta_handle = 'longhairwomen'

print ('Crawling for user handle: ', user_insta_handle, '...')
push_url_to_queue(user_insta_handle)
print ('Creating directory if does not exists')
isExist = os.path.exists(user_insta_handle)
if not isExist:
   os.makedirs(user_insta_handle)
   print("The new directory is created!")
print ('Total URLs Populated: ', len(url_queue))

for i in range(0, len(url_queue)):
    url = url_queue[i]
    page = requests.get(url)
    f_ext = os.path.splitext(url)[-1]
    f_name = './'+user_insta_handle+'/picture-'+str(i)+'.jpg'
    with open(f_name, 'wb') as f:
        f.write(page.content)
print('All photos stored in local disk !!')